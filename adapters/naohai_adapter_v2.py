...
class NaohaiAdapterV2:
    """Backward-compatible adapter used by existing unit tests."""
    ...
    def init_monitoring(
        ...
        *,
        ...
        self.monitoring.attach_event_bus(
            self.event_bus,
    ...
    def _default_plugin_dir(self) -> str:
        ...
        src_plugins = repo_root / "src" / "plugins"
        if src_plugins.exists():
            return str(src_plugins)
        return str(repo_root / "plugins")

    def init_plugins(self, plugin_dir: Optional[str] = None) -> None:
        if self.plugin_manager is not None:
            return
        self.plugin_manager = PluginManager(plugin_dir or self._default_plugin_dir())
    ...
    def init_plugins_from_config(self, config: Dict[str, Any], services: Optional[Dict[str, Any]] = None) -> None:
        ...
        plugin_dir = plugins_cfg.get("dir") or self._default_plugin_dir()
        ...
        self._loop_thread.run(
            self.plugin_manager.load_plugins(
                enabled_modules=list(plugin_configs.keys()),
